...
| M  | Static-analysis struct fixup catalog        | Prereq for retrying P2  | ~3 sessions |
...
## M — Static-analysis struct-fixup catalog (prereq for retrying Stage 11 Phase 2)

**Problem.** Stage 11 Phase 2 (`36c131a`, reverted in `b28352b`) tried
to delete the runtime byte-transform bodies of
`portFixupSprite/Bitmap/MObjSub/STRUCT_U{16,32}/FTAttributes/RawTextureBSWAP32`
on the premise that torch already pre-transforms every relevant byte
range at extraction. False premise: torch only pre-transforms the
ranges listed in `port/test/struct_fixup_catalog.json` (125 of 2,132
files; 2,592 entries total), and the catalog is generated by **runtime
capture** (`SSB64_FIXUP_CAPTURE=1` walking through scenes the captor
visits). Any scene the capture run missed produces no entries — torch
skips those ranges — runtime fallback used to compensate — Phase 2
deleted the fallback → visual regressions in catalog-uncovered scenes
(Link's sword hit effect, 1P stage-clear results screen, "GAME SET"
title-card).

The reverted state is fine: catalog stays as a perf optimization
(skip the runtime body when torch already did it for this specific
offset), runtime body is the source of truth.

**To retry Phase 2 safely**, replace the runtime-capture catalog with
a static-analysis catalog that traces every `portFixup<Family>(addr)`
call site in `decomp/src/**/*.c` to a `(file_id, byte_offset_in_file)`
tuple. Outline:

1. Parse decomp source (libclang AST or regex + targeted disambiguation).
2. For each call site, determine the type of `addr`:
   - Direct pointer arithmetic on a known reloc-file base
     (`(u8*)file_data + 0x300`) — trivial.
   - `PORT_RESOLVE(token)` of a reloc-token field — must resolve
     the token at extraction time (torch knows the token table from
     `RelocFile::InternChain`/`ExternChain`).
   - Cross-file refs via `ExternFileIds` — must walk through the
     dependent file's chain.
   - Heap-built sprites that happen to fall back to the runtime path
     — must NOT be in the catalog.
3. Emit `port/test/struct_fixup_catalog.json` deterministically; CI
   gate (follow-up I) ensures regen-on-source-change.
4. Re-extract assets; verify torch's `ApplyStructFixupsInPlace` covers
   100% of runtime call sites.
5. Now Phase 2's deletion is safe — every call site has a
   `fixup_torch_already_did_it` hit, runtime body is genuinely dead.

**Effort estimate**: ~3 sessions. Step 2 is the load-bearing one — many
call sites use indirection (struct-of-struct lookup, function-passed
sprite pointer, etc.) that requires data-flow analysis, not just
syntactic matching.

**Defer until** modder demand for "uniform post-fixup byte format
across all files" exceeds the analyzer cost. Today the runtime
fallback gives modders a single consistent post-pass1 format for
struct sub-resource overrides; that's the simpler contract.
